...
<Card class="!overflow-visible">
	...
	<CardContent class="p-0">
		<Table containerClass="!overflow-visible">
			<TableHeader>
				<TableRow>
					<TableHead class="pl-5">Turn</TableHead><TableHead>Model</TableHead><TableHead
						>Activity</TableHead
					><TableHead>Input</TableHead><TableHead>Cached</TableHead><TableHead>Output</TableHead
				...
				{#each session.turns as turn, index (turn.id)}
					...
					{#if turn.user_message && (index === 0 || session.turns[index - 1].user_message !== turn.user_message)}
						<TableRow class="bg-primary/15 hover:bg-primary/25">
							<TableCell colspan={7} class="px-5 py-3">
								...
								<p class="mt-1 whitespace-pre-wrap text-xs {messageExpanded ? '' : 'line-clamp-3'}">
									{turn.user_message}
								</p>
								{#if turn.user_message.length > 240}
									<Button
										...
										>{messageExpanded ? 'Show less' : 'Show more'}</Button
									>
								{/if}
							</TableCell>
						</TableRow>
					...
					<TableRow>
						...
						<TableCell>
							{#if model}
								<div class="flex items-center gap-1.5">
									...
									<button
										...
										><InfoIcon size={13} class="text-muted-foreground" /><span
											role="dialog"
							...
							{:else}<span class="text-xs text-muted-foreground">Unknown model</span>{/if}
						</TableCell>
						<TableCell class="max-w-48"
							><p class="truncate text-xs">{turn.types.join(' + ') || '—'}</p>
							{#if turn.reason}<p class="text-[11px] text-muted-foreground">
									{turn.reason}
								</p>{/if}</TableCell
						>
						<TableCell
						...
						<TableCell class="pr-5 text-right text-xs font-medium"
							>{formatCost(turnCost(turn))}</TableCell
						>
					</TableRow>
				{:else}
					<TableRow
