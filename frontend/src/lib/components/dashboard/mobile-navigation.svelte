			...
			<ScrollArea class="min-h-0 flex-1">
				<nav class="space-y-5 p-4" aria-label="Dashboard navigation">
					...
					<section aria-labelledby="mobile-sessions-heading" aria-busy={isLoading}>
						...
						<div class="space-y-1">
							<button
								class="min-h-10 w-full rounded-md px-2.5 py-2 text-left text-xs transition-colors hover:bg-muted {selectedSessionKey ===
								null
									? 'bg-primary/15 font-medium text-foreground'
									: 'text-muted-foreground'}"
							...
							{#if isLoading}
								<div class="space-y-2 px-2 py-1" aria-label="Loading sessions">
									{#each Array.from({ length: 5 }, (_, index) => index) as index (index)}
										<div class="h-12 animate-pulse rounded-md bg-muted"></div>
									{/each}
								</div>
							{:else}
								{#each pricedSessions as { session, cost } (sessionKey(session.source, session.session_id))}
									<button
										class="min-h-12 w-full rounded-md border border-transparent px-2.5 py-2.5 text-left transition-colors hover:bg-muted {selectedSessionKey ===
